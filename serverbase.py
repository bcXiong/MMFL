...
class Server:
    ...
    def aggregate_grads(self):  # 总和梯度
        assert (self.users is not None and len(self.users) > 0)  # 其返回值为假，就会触发异常
        for param in self.model.parameters():
            param.grad = torch.zeros_like(param.data)
    ...
    def add_parameters(self, user, ratio):
        ...
        for server_param, user_param in zip(self.model.parameters(), user.get_parameters()):
            server_param.data = server_param.data + user_param.data.clone() * ratio  # ???为什么是加上

    def aggregate_parameters(self):
        assert (self.users is not None and len(self.users) > 0)

        for param in self.model.parameters():
            param.data = torch.zeros_like(param.data)  # 返回一个全0的Tensor，其维度与input相一致
    ...
    def test(self):
        ...
        losses = []
        for c in self.users:
            ct, ns = c.test()  # 得到test_acc, y.shape[0] 且Test Accuracy: test_acc / y.shape[0]
    ...
    def train_error_and_loss(self):
        ...
        for c in self.users:
            ...
            ct, cl, ns = c.train_error_and_loss()  # 得到 train_acc, loss , self.train_samples
    ...
    def evaluate_one_step(self):
        for c in self.users:
            c.train_one_step()
        stats = self.test()    # 得到ids, num_samples, tot_correct(test准确率汇总)
